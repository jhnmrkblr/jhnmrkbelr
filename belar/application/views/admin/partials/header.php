...
<body class="bg-white">
    ...
    <nav class="navbar navbar-expand-md navbar-light bg-light sticky-top mb-3">
        <div class="container-fluid">
            <a class="navbar-brand" href="<?php echo base_url().'admin/home';?>">FeedMeUp (Admin Panel)</a>
            <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarRes">
                <span class="navbar-toggler-icon"></span>
            </button>

             <?php
        // Read current maintenance mode state
        $file = APPPATH . 'config/maintenance.json';
        $is_maintenance = false;
        if (file_exists($file)) {
            $data = json_decode(file_get_contents($file), true);
            $is_maintenance = !empty($data['maintenance_mode']);
        }
    ?>

    <form action="<?php echo base_url('admin/home/toggle_maintenance'); ?>" method="post">
        <div class="col-md-12">
            <input class="btn btn-success mb-3" 
                   type="checkbox" 
            ...
            <label class="btn btn-success mb-3" for="maintenanceSwitch">
                <strong>
                    <?php echo $is_maintenance ? 'Maintenance Mode: ON' : 'Maintenance Mode: OFF'; ?>
                </strong>
            </label>
        </div>
    </form>
